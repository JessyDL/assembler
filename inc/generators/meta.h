		...
		for(auto file_it = std::begin(all_files); file_it != meta_end; ++file_it) {
			...
			if(auto file_content = psl::utility::platform::file::read(file); file_content) {
				...
				for(auto const& platform_filepath : files) {
					...
					auto final_filepath = psl::utility::platform::directory::to_generic(
					  std::filesystem::relative(filepath, lib_dir).string());
					auto final_metapath = psl::utility::platform::directory::to_generic(
  ...
  public:
	void generate_meta(psl::string in_path,
		...
		for(auto const& [input, output] : files) {
			if(psl::string_view dir {output->data(), output->rfind('/')};
			   !psl::utility::platform::directory::exists(dir))
