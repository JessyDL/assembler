...
class importer_result_t {
  public:
	...
	auto&& consume() { return std::move(operations); }
...
class importer_t {
  ...
  public:
	...
	auto map_extension(psl::string const& extensions, psl::array<std::shared_ptr<importer_base_t>> const& importers)
	  -> bool;
	auto map_extension(psl::array<psl::string> const& extensions, std::shared_ptr<importer_base_t> const& importers)
	  -> bool;
	...
	auto ignore_extension(psl::string const& extension) -> void { m_IgnoredExtensions.push_back(extension); }
