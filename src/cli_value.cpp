...
value_base::value_base(const psl::string & name, const psl::string & cmd, std::optional<psl::char_t> short_cmd, std::optional<psl::string> hint, bool required)
...
psl::string value_base::help(uint32_t indent) const
{
	...
	res += psl::string(indent *2, ' ') + m_Name + ((required())?"*":"")+ psl::string(24u - m_Name.size() - (uint64_t)required(), ' ') + "[" + ((m_ShortCmd) ? "-" + psl::string(1u, m_ShortCmd.value()) + ", --" : "--") + m_Cmd + ']' + psl::string(24 - cmd_size, ' ') + "|  " + m_Hint.value_or("") + '\n';
	return res;
}
