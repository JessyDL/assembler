...
class glslang {
  ...
  public:
	...
	enum class type : uint8_t { unknown = 100, vert = 0, tesc = 1, tese = 2, geom = 3, frag = 4, comp = 5 };
	glslang()						   = delete;
	~glslang()						   = delete;
	glslang(glslang const&)			   = delete;
	glslang(glslang&&)				   = delete;
	glslang& operator=(glslang const&) = delete;
	glslang& operator=(glslang&&)	   = delete;

	static bool compile(psl::string_view compiler_location,
