...
bool parameter_pack::validate()
{
	...
	for(const auto& value : m_Values)
	{
		if(value->is_parameter_pack() && value->name().empty() && !value->short_command() && value->command().empty())
		{
			...
			ppack->get().validate();
			m_FlattenedValues.insert(std::end(m_FlattenedValues), std::begin(ppack->get().m_FlattenedValues), std::end(ppack->get().m_FlattenedValues));
			m_Callbacks.insert(std::end(m_Callbacks), std::begin(ppack->get().m_Callbacks), std::end(ppack->get().m_Callbacks));
		}
		else
		{
	...
	{
		auto it = std::find_if(std::begin(m_FlattenedValues), std::end(m_FlattenedValues), [&value](const std::shared_ptr < cli::value_base>& entry)
							   {
								   return entry != value && (entry->command() == value->command() || (entry->short_command() && entry->short_command() == value->short_command()));
							   });
		if(it != std::end(m_FlattenedValues))
		{
			utility::terminal::set_color(utility::terminal::color::RED);
			psl::cerr << psl::to_platform_string("duplicate parameter found: the param '" + (*it)->name() +"' was equal to the param '" + value->name());
...
bool parameter_pack::parse(const std::vector<token>& tokens, std::vector<cli::parameter_pack*>& parsed_packs)
{
	if(!validate())
	{
		utility::terminal::set_color(utility::terminal::color::RED);
	...
	for(const auto& v : m_FlattenedValues)
	{
		if(v->required())
		{
			auto it = std::find_if(std::begin(tokens), std::end(tokens), [&v](const token& tokenPair)
								   {
									   return (tokenPair.key.size() == 1 && tokenPair.key[0] == v->short_command()) || tokenPair.key == v->command();
								   });
	...
	{
		const auto& token_v = tokens[i];
		auto it = std::find_if(std::begin(m_FlattenedValues), std::end(m_FlattenedValues), [&token_v](const std::shared_ptr<value_base>& value)
							   {
								   return (token_v.key.size() == 1 && token_v.key[0] == value->short_command()) || token_v.key == value->command();
							   });
		...
		if(it != std::end(m_FlattenedValues))
		{
			if((*it)->is_parameter_pack())
			{
				...
				auto success = next_pack.parse(std::vector<token>{std::begin(tokens) + i + 1, std::end(tokens)}, parsed_packs);
		...
		else
		{
			...
			{
				utility::terminal::set_color(utility::terminal::color::RED);
				psl::cerr << psl::to_platform_string("unknown parameter: '" + psl::string((token_v.key.size() > 1) + 1, '-') + psl::string(token_v.key) + "' detected\n");
				utility::terminal::set_color(utility::terminal::color::WHITE);
			}
		}
	}
...
bool parameter_pack::parse(psl::string_view parameters)
{
	...
	while(next_parameter_n != psl::string_view::npos && next_parameter_n < parameters.size())
	{
		...
		if(next_param_n == psl::string_view::npos || next_param_n >= parameters.size() || parameters[next_param_n] == '-')
		{
